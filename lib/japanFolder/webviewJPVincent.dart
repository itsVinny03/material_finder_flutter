...
class _SoftwareWebViewScreenState extends State<SoftwareWebViewScreenJP> with WidgetsBindingObserver {
  ...
  Future<void> _setupInputFieldDetection() async {
    if (webViewController != null) {
      String jsCode = '''
(function barcodeButtonSetup() {
  const parentTargetIds = ['withdrawInventory'];
  const trackedInputs = {};
  ...
  function attachBarcodeScannerButton(input, id) {
    if (!input) return;
    ...
    if (shouldShow && !input.dataset.hasBarcodeButton) {
      ...
      button.onclick = function(e) {
        ...
        if (window.flutter_inappwebview) {
          window.flutter_inappwebview.callHandler('openBarcodeScanner', id);
        }
      };
      
      container.appendChild(button);
      trackedInputs[id] = { button, container };
    }
    
    if (!shouldShow && trackedInputs[id]) {
      const { container } = trackedInputs[id];
      if (container && container.parentNode) {
        input.removeAttribute('data-has-barcode-button');
        container.parentNode.insertBefore(input, container);
        container.remove();
      }
      delete trackedInputs[id];
    }
  }

  function updateParentBarcodeScannerButtons() {
    parentTargetIds.forEach(id => {
      const input = document.getElementById(id);
      attachBarcodeScannerButton(input, id);
    });
  }

  // Inject code into all iframes (cross-origin)
  function injectIntoIframes() {
    Array.from(document.getElementsByTagName('iframe')).forEach((iframe, index) => {
      try {
        iframe.contentWindow.postMessage({
          type: 'INJECT_BARCODE_SCANNER',
          targetIds: ['newMaterialTag'],
          iframeIndex: index
        }, '*');
      } catch (e) {
  ...
  // Initial setup
  updateParentBarcodeScannerButtons();
  injectIntoIframes();

  // Listen for messages from iframes
  window.addEventListener('message', function(event) {
    if (event.data && event.data.type === 'BARCODE_SCANNER_CLICKED') {
      const { inputId } = event.data;
      if (window.flutter_inappwebview) {
        window.flutter_inappwebview.callHandler('openBarcodeScanner', inputId);
      }
    }
  });
